...
def runtime_metrics(
    ...
    return load_widths(metrics_path)
    ...
    while position < len(used):
        compound = next(
            (
                ...
                if tuple(
                    runtime_metrics(metrics_path)[1][character] for character in text
                )
...
def render_action_chunk(
    ...
    for code in codes:
        ...
        if (
            len(cell) != 32
            or code >= len(runtime_metrics(metrics_path)[0])
            or not runtime_metrics(metrics_path)[0][code]
        ):
        ...
        for row in range(12):
            ...
            rows[row] |= word >> x
        x += runtime_metrics(metrics_path)[0][code]
...
def render_compound_glyph(
    ...
    for character in text:
        code = runtime_metrics(metrics_path)[1][character]
        start = code * 32
        cell = font16[start : start + 32]
        if (
            len(cell) != 32
...
def build_compound_glyph_runtime(
    font16: bytes,
    metrics_path: Path = METRICS_PATH,
) -> bytes:
    ...
    for text in COMPOUND_TEXT:
        payload.extend(render_compound_glyph(font16, text, metrics_path))
    payload.extend(bytes((-len(payload)) % 4))
...
def build_action_atlas(
    ...
    for _offset, name in ACTION_BLOCKS:
        codes = encode_ascii(asset_ascii(name, asset), metrics_path=metrics_path)
        chunks = []
        position = 0
        while position < len(codes):
            ...
            for end in range(len(codes), position, -1):
                ...
                try:
                    render_action_chunk(font16, candidate, metrics_path)
                except ValueError:
                    continue
                chunk = candidate
                break
    ...
    for chunk, index in indices.items():
        cell, width = render_action_chunk(font16, chunk, metrics_path)
...
def compound_widths(metrics_path: Path = METRICS_PATH) -> bytes:
    values = []
    for text in COMPOUND_CODES:
        values.append(
            sum(
                runtime_metrics(metrics_path)[0][
                    runtime_metrics(metrics_path)[1][character]
                ]
                for character in text
            )
        )
...
def build_vwf_wrapper(
    ...
    action_widths_address = (
        widths_address + len(runtime_metrics(metrics_path)[0]) + 3
    ) & ~3
    ...
    payload.extend(compound_widths(metrics_path))
    payload.extend(bytes((-len(payload)) % 4))
    return bytes(payload)
...
def build_config(
    ...
    action_atlas, action_widths, action_records = build_action_atlas(
        font16, metrics_path, asset
    )
    compound_runtime = build_compound_glyph_runtime(font16, metrics_path)
    data = bytearray(original)

    cave = build_vwf_wrapper(action_widths, metrics_path)
    ...
    magic_sort_table = b"".join(
        sort_record(name, metrics_path, asset) for name in MAGIC_SORT_BLOCKS
    )
    item_sort_table = b"".join(
        sort_record(name, metrics_path, asset) for name in ASSIST_BLOCKS
    )
    require(
        data,
        MAGIC_SORT_TABLE_FILE,
    ...
    for offset, name in FOOTER_BLOCKS:
        values = encode_ascii(
            asset_ascii(name, asset), compounds=True, metrics_path=metrics_path
        )
        if len(values) > 9:
